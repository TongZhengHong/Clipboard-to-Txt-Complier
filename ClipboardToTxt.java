...
public class ClipboardToTxt implements ActionListener, 
	...
	// Bottom section
	private JCheckBox autoSaveCheckBox = new 
		JCheckBox("Auto save file if clipboard has multiple lines");
	...
	private void createLayout() {
		...
		clipboardPanel.add(clipBoardTextArea);
		currentFilePanel.add(currentFileTextArea);
	...
	private void setupListeners() {
		...
		outputFolderTextField.getDocument().addDocumentListener(new DocumentListener() {
			public void insertUpdate(DocumentEvent ev) { updateFileExplorerTree(); }
			public void removeUpdate(DocumentEvent ev) { updateFileExplorerTree(); }
		...
		if (currentDirectory == null) fileExplorerTree = new FileExplorerTree();
		else fileExplorerTree = new FileExplorerTree(currentDirectory.getAbsolutePath());
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(command.equals("Start")) {
			...
			if (parentDirectory == null || !parentDirectory.exists()) return;
			...
			if (inputDialogResult == null) return; 
			String renamedString = inputDialogResult.toString();

			if (renamedString.isEmpty() || 
				renamedString.equals(selectedFile.getName())) return;
			if (!renamedString.contains(".txt")) renamedString += ".txt";
	...
	public void onClipBoardUpdate(String clipboardText) {
		...
		if (currentText.isEmpty()) currentText += clipboardText;
		else currentText += "\n" + clipboardText;
